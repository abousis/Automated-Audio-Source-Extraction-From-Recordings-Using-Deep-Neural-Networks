...
class DataGenerator(tensorflow.keras.utils.Sequence):
    def __init__(self, steps_per_epoch, tracks_in_batch, subsets, split):
        ...
        self.mus = musdb.DB(root=os.path.join(self.dir_path, self.musdb_folder), subsets=subsets, split=split)
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.tracks_in_batch):
            ...
            for j in range(self.chunk_per_track):
                ...
                self.mixture_train_data[:, j:j + 1] = np.reshape(mixture_padded[:, start_index:end_index],
                                                                 (self.num_ft_bins, 1), order="F")
                self.ibm_train_label[:, j:j + 1] = np.reshape(ibm_padded[:, start_index:end_index],
                                                              (self.num_ft_bins, 1), order="F")
